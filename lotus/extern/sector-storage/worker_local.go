...
import (
	...
	"github.com/filecoin-project/go-fil-markets/ddfs-sdk/addpiece" //DD add
)
...
/*DD begin*/
func (l *LocalWorker) AddPieceExt(ctx context.Context, sector storage.SectorRef, epcs []abi.UnpaddedPieceSize, sz abi.UnpaddedPieceSize, r io.Reader, extInfo map[string]interface{}) (storiface.CallID, error) {
	sb, err := l.executor()
	if err != nil {
		return storiface.UndefCall, err
	}
	...
	return l.asyncCall(ctx, sector, AddPiece, func(ctx context.Context, ci storiface.CallID) (interface{}, error) {
		addpiece.SetCtxFromExtInfo(&ctx, extInfo)
